...
export default function UploadCustom({ allowedTypes = ['image'], galleryType: defaultGalleryType = null, onUpload = () => {} }) {
  ...
  // Use the galleryType passed from parent (auto-selected based on sidebar)
  const galleryType = defaultGalleryType;

  const handleFiles = async (files) => {
    if (!galleryType) {
      setError('Please select an image type from the sidebar first.');
      return;
    }
    setDragActive(false);
    setPreview(null);
  ...
  // Map galleryType number to readable name
  ...
  return (
  <div>
      {/* Display selected type instead of combobox */}
      {galleryType && (
        <div className="mb-3 p-3 bg-blue-50 border border-blue-200 rounded-lg">
          <span className="text-sm font-medium text-blue-900">
            Uploading as: <strong>{getTypeName(galleryType)}</strong>
          </span>
        </div>
      )}
      ...
      >
        ...
        {!isTypeSelected && <span className="text-red-600 mt-2">Select an image type from the sidebar before uploading.</span>}
